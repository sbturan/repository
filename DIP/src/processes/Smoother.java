...
public class Smoother extends Process {
    ...
    public Smoother(BufferedImage img, int filterSize) throws Exception {
        if (filterSize <= 2 || filterSize % 2 != 1) {
            throw new Exception("Hatalı Filtre Boyutu");
        }
        this.filePath = filePath;
    ...
    public Smoother(int filterSize, FilterType filterType)
    {
    ...
    public enum FilterType {MedianFilter, AvarageFilter }
    public FilterType filterType = FilterType.MedianFilter;
    
    private void processMedianFilter() {
    ...
    private void processAvarageFilter() {
        for (int i = filterSize / 2; i < img.getWidth() - (filterSize / 2); i++) {
            for (int j = filterSize / 2; j < img.getHeight() - (filterSize / 2); j++) {
